...
- `src/main/java/com/takumi/takumimod/`
  ...
  - **段ボールの剣の強化アップグレード**: `ModItems`はこのクラスを別ティアで二重登録した`cardboard_sword_reinforced_stone`（石強化、`Tiers.STONE`）・`cardboard_sword_reinforced_deepslate`（深層岩強化、`item/CardboardTiers.java`の独自`Tier`実装）も持つ。攻撃速度はいずれも3.6を維持（`attackDamageModifier`引数は全ティア共通で`3`のまま、`Tier#getAttackDamageBonus()`だけがティアごとに変わる設計）。石強化は攻撃力が石の剣相当（コブルストーン2個+段ボールの剣、シェイプレスレシピ）。深層岩強化は「石強化より攻撃力0.5低いが耐久力はかなり高い」というユーザー指定のバランスで、`CardboardTiers.DEEPSLATE_REINFORCED`（`getAttackDamageBonus()=0.5`、耐久300、修理素材はコブル深層岩）として実装（コブル深層岩2個+段ボールの剣）。どちらも段ボールの剣からの並列アップグレードで、段階的な多段強化ではない
  ...
  - **強化バリアントのテクスチャ切り替え**: `CardboardSwordItemRenderer`はレンダラーインスタンスを`cardboard_sword`/`cardboard_sword_reinforced_stone`/`cardboard_sword_reinforced_deepslate`の3アイテムで共有している（Forge版は`CardboardSwordItem`の各インスタンスが同じレンダラークラスを個別登録、NeoForge版は`TakumiModClient`の`RegisterClientExtensionsEvent`で3アイテムまとめて同一インスタンスを登録）。ベーステクスチャは`renderByItem`内で`stack.is(ModItems.CARDBOARD_SWORD_REINFORCED_DEEPSLATE.get())`等のアイテム種別判定によって切り替えており、アイテムクラス側に状態を持たせていない（NeoForge版は`CardboardSwordItem`サブクラスを使わず素の`SwordItem`なのでこの判定方式でないと両バージョンで実装を揃えられない）。発光レイヤー（`cardboard_sword_glow.png`）は3バリアント共通、ベース刃テクスチャのみ`cardboard_sword_reinforced_stone.png`（薄い灰色寄りにブレンド）・`cardboard_sword_reinforced_deepslate.png`（濃い黒寄りにブレンド）を用意。ブレンドは刃部分（alpha=180の半透明ピクセルのみ）に対して行い、柄・鍔（alpha=255の不透明ピクセル）は元の色のまま
