...
- `src/main/java/com/takumi/takumimod/`
  ...
  - **段ボールの剣の強化チェーン**: `ModItems`はこのクラスを別ティアで12回（段ボールの剣本体+石系統5段階+深層岩系統5段階）登録している。2つの独立した連鎖アップグレード系統を持つ:
    - **石系統**（段ボールの剣 → `_stone`(`Tiers.STONE`) → `_iron`(`Tiers.IRON`) → `_gold`(独自Tier) → `_diamond`(独自Tier) → `_emerald`(独自Tier) → `_netherite`(独自Tier)）。stone/ironだけvanillaの`Tiers`をそのまま流用、gold以降はユーザー指定の攻撃力（ツールチップ表示値: 金9・ダイヤ10・エメラルド12・ネザライト16）に合わせるため独自Tierに変更した（vanillaの`Tiers.GOLD`等は攻撃力ボーナスが固定でユーザー要望の値にできないため）
    - **深層岩系統**（段ボールの剣 → `_deepslate` → `_obsidian` → `_crying_obsidian` → `_sculk` → `_end` → `_nether_star`、全段階が`item/CardboardTiers.java`の独自`Tier`）。深層岩強化は「石強化より攻撃力0.5低いが耐久力はかなり高い」という明示的なユーザー指定バランス、泣く黒曜石以降もユーザー指定の攻撃力（ツールチップ表示値: 泣く黒曜石7・スカルク9・エンド18・ネザースター24）を反映済み。黒曜石強化のみステータス未指定のため仮値のまま
    - **攻撃力の逆算方法**: 表示攻撃力 = 1（素手基礎値）+ `attackDamageModifier`引数（全ティア共通で`3`固定）+ `Tier#getAttackDamageBonus()`。ユーザーから「表示攻撃力X」と指定された場合は`getAttackDamageBonus() = X - 4`で`CardboardTiers`に設定する
    - 各レシピはシェイプレスの「1つ前の剣1個+素材2個」で統一。段階を飛ばして直接上位ティアを作ることはできない
    - 攻撃速度はどのティアも3.6を維持（`attackDamageModifier`引数は全ティア共通で`3`のまま、`Tier#getAttackDamageBonus()`だけがティアごとに変わる設計）
    - **`Tier`インターフェースはバージョンで形が違う**: Forge 1.20.1は旧式の`int getLevel()`（採掘レベル）を要求し`BlockTags.INCORRECT_FOR_*`系のタグは存在しない。NeoForge 1.21.1は逆にタグベースの`TagKey<Block> getIncorrectBlocksForDrops()`を要求し`getLevel()`は無い。同じ`Tier`実装コードをコピペでは両対応できず、バージョンごとに別メソッドをオーバーライドする必要がある（剣は採掘に使わないためどちらの値も実質ダミーだが、インターフェースの契約上省略できない）
  - **強化バリアントのテクスチャ切り替え**: `CardboardSwordItemRenderer`はレンダラーインスタンスを全12アイテムで共有している（Forge版は`CardboardSwordItem`の各インスタンスが同じレンダラークラスを個別登録、NeoForge版は`TakumiModClient`の`RegisterClientExtensionsEvent`で全アイテムまとめて同一インスタンスを登録）。ベーステクスチャは`renderByItem`内でアイテムの**レジストリ名からテクスチャパスを直接導出**する方式（`textures/item/<レジストリのpath>.png`）に統一している。アイテムIDとテクスチャファイル名を完全一致させる命名規約にすることで、バリアントが増えるたびにレンダラー側へ判定分岐を追加する必要がなくなる（以前は`stack.is(...)`のif連鎖だったが、ティア数が増える見込みだったため早い段階でこの方式にリファクタした）。発光レイヤー（`cardboard_sword_glow.png`）は全バリアント共通、ベース刃テクスチャだけティアごとに用意し、刃部分（alpha=180の半透明ピクセルのみ）を各ティアのテーマカラーにブレンドしている（柄・鍔=alpha=255の不透明ピクセルは元の色のまま）
...
- **索敵距離・追跡の持続性はユーザー指定でかなり広い**: `Attributes.FOLLOW_RANGE`を32→500に設定（`createAttributes()`）。`TargetGoal#canContinueToUse()`（`NearestAttackableTargetGoal`はもちろん、`DefendOwnerTargetGoal`/`AssistOwnerTargetGoal`も`TargetGoal`を継承しているため同様）はこの値を「対象を見失ったとみなす距離」として使うため、500に上げたことで実質的に索敵・追跡ともに広範囲になる。`NearestAttackableTargetGoal`のコンストラクタは`mustSee`引数もfalseに変更済み（元はtrue）——trueのままだと壁などで視線が一定時間（`unseenMemoryTicks`）切れただけでターゲットを見失う仕様のため、「一度敵対したら追いかけ続ける」という要望を満たすためにはこちらもfalseにする必要があった。`DefendOwnerTargetGoal`/`AssistOwnerTargetGoal`は元々`super(box, false)`でmustSee=falseだったため変更不要
  - 500ブロックは通常のシミュレーション距離・チャンクロード範囲を大きく超える値のため、実際には「読み込まれているチャンク内の対象なら追跡し続ける」程度の効果になる点に留意（未ロードの対象までは物理的に索敵できない）
- 新しいアイテム/エンティティを追加したら、`en_us.json` と `ja_jp.json` の両方にローカライズキーを追加すること
- `README.md` は日本語で、ユーザー向けの機能一覧・レシピを記載。実装を変更したら追従させる
- **装備システム**: `CardboardBoxEntity#mobInteract` でオーナーのシフト右クリックのみ受け付ける。手が空でなければそのアイテム1個を装着する。装着先の`EquipmentSlot`は`resolveEquipSlot`で判定し、`ArmorItem`（ヘルメット/チェストプレート/レギンス/ブーツ）なら持っている手に関わらず`ArmorItem#getEquipmentSlot()`が返す防具スロットへ、それ以外のアイテムは`hand`引数（メインハンド/オフハンド）通りに装着する。既存装備は player のインベントリへ（入らなければドロップ）。手が空なら`findUnequipSlot`で「まずhand引数に対応するメインハンド/オフハンドスロット、無ければ防具スロット（足→脚→胴→頭の順）」を探して装備を外し手に渡す
  - 見た目の反映には `CardboardBoxModel implements ArmedModel`（`translateToHand`で右腕/左腕にアイテム位置を合わせる）+ `CardboardBoxRenderer` に `ItemInHandLayer` を追加。防具は機能的には装着できるが、モデルに頭部パーツが無い（胴体キューブがそのまま頭を兼ねる形状）ため防具レイヤーの見た目には対応していない（装備自体は正常に機能し、攻撃力・防御力などのステータスには反映される）
- **飛行中の攻撃**: `CardboardMeleeAttackGoal`（`MeleeAttackGoal`のサブクラス）と`FlyAtTargetGoal`はどちらも`Flag.MOVE`を宣言しているため同時実行できない（GoalSelectorの仕様上、同じフラグを持つゴールは優先度が高い方だけが動く）。そのため飛行中に殴らせたい場合はメレー攻撃任せにせず、`FlyAtTargetGoal`自身の`tick()`内で射程判定・クールダウン管理・`doHurtTarget`呼び出しまで完結させている（`ATTACK_INTERVAL_TICKS`で連打を防止）
- **攻撃頻度**: `MeleeAttackGoal`のデフォルト攻撃間隔は20tick（1秒）で固定。**`getAttackInterval()`をオーバーライドしても効果が無い罠がある**——`resetAttackCooldown()`が`getAttackInterval()`を呼ばず`this.adjustedTickDelay(20)`を直接ハードコードしており、実際のクールダウンを持つ`ticksUntilNextAttack`フィールドも`private`でサブクラスから触れない（最初これで実装して「変わらない」と指摘され気付いた）。正しい対処は`entity/goal/CardboardMeleeAttackGoal.java`で`checkAndPerformAttack(...)`自体を丸ごとオーバーライドし、独自の`attackCooldown`フィールド（10tick=0.5秒）で攻撃タイミングを完全に自前管理すること。`checkAndPerformAttack`のシグネチャはバージョンで異なる（Forge 1.20.1は`(LivingEntity, double distToTargetSqr)`、NeoForge 1.21.1は`(LivingEntity)`のみで内部の`canPerformAttack`が距離判定を行う）。`FlyAtTargetGoal.ATTACK_INTERVAL_TICKS`も同じく20→10tickに揃えて飛行突撃時の攻撃頻度も合わせて上げている。コンストラクタ引数は`Mob`ではなく`PathfinderMob`（vanillaの`MeleeAttackGoal`自体がその型を要求するため）
- **`FlyAtTargetGoal.canUse()`は接地判定を持たない（ユーザー指定で撤廃）**: 元は`onGround()`必須だった（一度「垂直速度がほぼ0であること」も追加してみたが、接地中のMobでも重力の残差で`getDeltaMovement().y`は`0`ちょうどにはならず`-0.08`前後になることが多く、厳しめの閾値`< 0.01`では接地判定がほぼ常にfalseになり突撃自体が発生しなくなる回帰を起こした経緯がある）。その後「空中にいても突進できるようにしてほしい」という要望で`onGround()`チェックごと削除した。`tick()`側は元々接地状態を前提にしたロジックを持たない（`setNoGravity(true)`にして目標方向へ`deltaMovement`を設定するだけ）ため、空中発動でも技術的な問題は無い
