...
- 独自`Tier`実装（`item/CardboardTiers.java`）で要求されるメソッドが違う。1.20.1は旧式の`int getLevel()`（採掘レベル）、1.21.1はタグベースの`TagKey<Block> getIncorrectBlocksForDrops()`（`BlockTags.INCORRECT_FOR_*`を返す）。同じコードを両バージョンにコピペすると片方が「オーバーライドすべき抽象メソッドが無い」でコンパイルエラーになる
- **データパックのフォルダ名が単数形になった**（重要・気付きにくい）: 1.21.1のバニラデータは`data/minecraft/recipe/`（単数形）・`data/minecraft/advancement/`（単数形）・`data/minecraft/loot_table/`（単数形）を使う。1.20.1以前は`recipes/`・`advancements/`・`loot_tables/`（複数形）だった。MODの`data/cardboard_pet/recipes/`を複数形のまま置いていたところ、**コンパイルは通り起動時のエラーも出ないまま、該当データパックの中身が丸ごと1件も読み込まれない**という発見しづらい形で壊れた（`RecipeManager`のレシピ総数がMOD追加前と1件も変わらないことでしか気付けない）。フォルダ名が存在しなくてもForge/NeoForgeは警告すら出さないので、`event.getServer().getRecipeManager().getRecipes()`を一時的にダンプして総数・名前空間を直接確認するまで気付けなかった
- **レシピJSONの`result`のキー名が`item`→`id`に変わった**: 1.20.1は`"result": {"item": "modid:foo", "count": 1}`。1.21.1は`"result": {"id": "modid:foo", "count": 1}`（`key`/`ingredients`内の個々のアイテム指定は`item`のままなので、resultだけが変則的に違う）。間違えたまま`data/cardboard_pet/recipe/`（単数形）に置くと`com.google.gson.JsonParseException: No key id in MapLike[...]`が出て該当レシピ1件がスキップされる（他のレシピ・MOD全体には影響しない、ログにエラーとして残るので気付きやすい）
